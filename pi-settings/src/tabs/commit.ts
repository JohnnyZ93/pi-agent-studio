...
<div class="tab-section">
  ...
  <div class="editor-card">
    ...
    <div class="hint">Model used to generate Git commit messages, in "provider/model" format. Leave as (default) to use pi's default model.</div>
    <label class="field-label">Language</label>
    <select id="cm-language">
      ${languages.map((l) => `<option value="${escHtml(l)}" ${l === data.commitLanguage ? "selected" : ""}>${escHtml(l)}</option>`).join("")}
    </select>
    <div class="hint">Language for generated Git commit messages.</div>
    <label class="field-label">Custom prompt</label>
    <textarea id="cm-prompt" class="ta" style="height:140px" placeholder="Custom system prompt for commit message generation. If empty, uses the default prompt.">${escHtml(data.commitMessagePrompt ?? "")}</textarea>
    <div class="btn-row">
